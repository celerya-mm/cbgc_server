...
class FormCertConsCreate(FlaskForm):
    ...
    certificate_year = IntegerField('Year', validators=[DataRequired("Campo obbligatorio!")], default=year_cert_calc())

    emitted = SelectField("certificato Emesso", choices=["SI", "NO"], default="NO")
...
class FormCertConsUpdate(FlaskForm):
    ...
    certificate_year = IntegerField('Year', validators=[DataRequired("Campo obbligatorio!")])

    emitted = SelectField("Emesso", choices=["SI", "NO"])

    cockade_id = IntegerField('ID Coccarda', validators=[Optional()])
    cockade_var = StringField('Nota ID Coccarda', validators=[Length(max=10), Optional()])
